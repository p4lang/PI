...
class ActionProfGroupMembership {
 public:
  ...
  struct Membership {
    ...
    struct Comp {
      bool operator()(const Membership &m1, const Membership &m2) {
        return (m1.id < m2.id);
      }
    };
  };
  ...
  void set_membership(std::vector<Membership> &&new_members);

  const std::vector<Membership> &get_membership() const;
...
class ActionProfMgr {
 ...
 private:
  ...
  Status group_add_members(pi::ActProf &ap, pi_indirect_handle_t group_h,
                           It first, It last) {
    for (auto it = first; it != last; ++it) {
      RETURN_IF_ERROR(group_add_member(ap, group_h, *it));
    }
    RETURN_OK_STATUS();
  }
  // NOLINTNEXTLINE(runtime/references)
  Status group_add_member(pi::ActProf &ap, pi_indirect_handle_t group_h,
                          pi_indirect_handle_t member_h);

  template <typename It>
  // NOLINTNEXTLINE(runtime/references)
  Status group_remove_members(pi::ActProf &ap, pi_indirect_handle_t group_h,
                              It first, It last) {
    for (auto it = first; it != last; ++it) {
      RETURN_IF_ERROR(group_remove_member(ap, group_h, *it));
    }
    RETURN_OK_STATUS();
  }
  // NOLINTNEXTLINE(runtime/references)
  Status group_remove_member(pi::ActProf &ap, pi_indirect_handle_t group_h,
                             pi_indirect_handle_t member_h);

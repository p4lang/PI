...
TEST(GetFv, U8) {
  ...
  for (size_t bitwidth = 1; bitwidth <= 8; bitwidth++) {
    for (uint32_t v = 0; v < (uint32_t) (1 << bitwidth); v++)  {
      ...
      TEST_ASSERT_EQUAL_UINT(fid, fv.fid);
...
TEST_GROUP_RUNNER(GetFv) {
  RUN_TEST_CASE(GetFv, U8);
  RUN_TEST_CASE(GetFv, U8_ExtraBits);
  RUN_TEST_CASE(GetFv, U8_BadInput);
  RUN_TEST_CASE(GetFv, U16);
  RUN_TEST_CASE(GetFv, U16_BadInput);
  RUN_TEST_CASE(GetFv, U32);
  RUN_TEST_CASE(GetFv, U32_BadInput);
  RUN_TEST_CASE(GetFv, U64);
  RUN_TEST_CASE(GetFv, U64_BadInput);
  RUN_TEST_CASE(GetFv, Ptr);
  RUN_TEST_CASE(GetFv, Ptr_BadInput);
}

void test_getfv() {
  RUN_TEST_GROUP(GetFv);
}
